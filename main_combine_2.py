...
class CombinedSkinCancerSystem:
    ...
    def train_segmentation_model(self, yaml_path):
        ...
        results = model.train(
            ...
            name="skin_cancer_segmentation_4",
            exist_ok=True
        )

        metrics = model.val()
        export_path = self.model_dir / "segmentation_model.pth"
        shutil.copy(str(self.results_dir / "skin_cancer_segmentation_4" / "weights" / "best.pt"),
                   export_path)
    ...
    def train_classification_model(self, yaml_path):
        ...
        results = model.train(
            ...
            save=True,
            project=str(self.results_dir),
            name="skin_cancer_classification_4",
            exist_ok=True
        )

        metrics = model.val()
        export_path = self.model_dir / "classification_model.pth"
        shutil.copy(str(self.results_dir / "skin_cancer_classification_4" / "weights" / "best.pt"),
                   export_path)
